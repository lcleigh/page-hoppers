...
export default function ReadingSummaryCard() {
    ...
    const fetchSummary = async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch('http://localhost:8080/api/reading-logs/summary', {
          headers: {
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {renderBook(summary.lastBook)}
              </div>
              <div className="grid grid-cols-2 gap-4 mt-2">
                <div className="bg-lemon/30 p-3 rounded-xl border-2 border-lemon text-center">
                  <div className="font-bold text-charcoal text-lg">{summary.booksThisMonth}</div>
                  <div className="text-sm text-coolgray">Books This Month</div>
                </div>
                <div className="bg-lemon/30 p-3 rounded-xl border-2 border-lemon text-center">
                  <div className="font-bold text-charcoal text-lg">{summary.booksThisYear}</div>
                  <div className="text-sm text-coolgray">Books This Year</div>
                </div>
                <div className="bg-lemon/30 p-3 rounded-xl border-2 border-lemon text-center col-span-2">
                  <div className="font-bold text-charcoal text-lg">{summary.totalBooks}</div>
