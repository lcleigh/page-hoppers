...
export default function ParentDashboard() {
  ...
  const handleChildLoginSubmit = async (pin: string, e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:8080/api/auth/child/login', {
        ...
        body: JSON.stringify({
          childId: selectedChildId, // numeric ID,
          pin: pin,
        }),
      });
      ...
      localStorage.setItem('childId', selectedChildId.toString());
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-powder relative">
      ...
      <div className="w-full max-w-2xl bg-lavender rounded-2xl shadow-lg p-8 flex flex-col items-center">
        <h1 className="text-4xl font-extrabold mb-4 text-bubblegum drop-shadow">
          Parent Dashboard
        </h1>
        <p className="mb-6 text-lg text-charcoal">
          Welcome! Here are your children:
        </p>

        <form
          onSubmit={handleAddChild}
          className="mb-8 w-full max-w-md bg-white/80 p-6 rounded-xl shadow flex flex-col gap-3 border-2 border-lavender"
        >
          <h2 className="text-xl font-semibold mb-2 text-bubblegum">
            Add a Child
          </h2>
          <input
            type="text"
        ...
        {loading ? (
          ...
          <div className="text-coolgray">
            No children found. Add a child above!
          </div>
        ) : (
          <ul className="w-full max-w-md space-y-4">
            {children.map((child) => (
              <li
                key={child.id}
                className="p-4 bg-white rounded-xl shadow flex flex-col sm:flex-row sm:items-center sm:justify-between border-2 border-sky"
              >
